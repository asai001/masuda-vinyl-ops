#!/usr/bin/env node
/**
 * Seed script for Clients Master (dev only)
 * Fixed settings:
 * - region: ap-northeast-1
 * - table : masuda-vinyl-ops-clients-master-dev
 * - data  : ./seed_clients_master.json (relative to this script)
 *
 * Options:
...
const DATA_URL = new URL("./seed_clients_master.json", import.meta.url);
...
async function main() {
  ...
  if (!Array.isArray(items)) {
    throw new Error("seed_clients_master.json must be a JSON array.");
  }
