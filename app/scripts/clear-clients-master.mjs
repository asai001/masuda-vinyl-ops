#!/usr/bin/env node
/**
 * Clear script for Clients Master (dev only)
 * Fixed settings:
 * - region: ap-northeast-1
 * - table : masuda-vinyl-ops-clients-master-dev
 *
 * Options:
 *   --dry-run              print count only (no deletes)
...
const TABLE_NAME = "masuda-vinyl-ops-clients-master-dev";
  ...
  while (true) {
    const res = await ddbDoc.send(
      new ScanCommand({
        ...
        ProjectionExpression: "orgId, clientId",
        ExclusiveStartKey: lastEvaluatedKey,
      }),
    );

    for (const item of res.Items ?? []) {
      if (item.orgId && item.clientId) {
        keys.push({ orgId: item.orgId, clientId: item.clientId });
      }
    }
...
async function batchDeleteAll(ddbDoc, keys) {
  ...
  for (let bi = 0; bi < batches.length; bi++) {
    ...
    process.stdout.write(`\r[${TABLE_NAME}] ${Math.min((bi + 1) * 25, keys.length)}/${keys.length} deleted...`);
  }
