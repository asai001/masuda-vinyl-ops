#!/usr/bin/env node
/**
 * Clear script for Sales Orders (受注管理) (dev only)
 * Fixed:
  ...
  while (true) {
    const res = await ddbDoc.send(
      new ScanCommand({
        ...
        ProjectionExpression: "orgId, salesOrderId",
        ExclusiveStartKey,
      }),
    );

    for (const item of res.Items ?? []) {
      if (item?.orgId && item?.salesOrderId) {
        keys.push({ orgId: item.orgId, salesOrderId: item.salesOrderId });
      }
    }
