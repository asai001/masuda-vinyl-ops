#!/usr/bin/env node
/**
 * Seed script for Materials Master (test dataset) (dev only)
 * Fixed:
 * - region: ap-northeast-1
 * - table : masuda-vinyl-ops-materials-master-dev
 * - data  : ./test_materials_master.json (relative to this script)
 *
 * Options:
 *   --dry-run
...
const DATA_URL = new URL("./test_materials_master.json", import.meta.url);
...
async function main() {
  ...
  if (!Array.isArray(items)) {
    throw new Error("test_materials_master.json must be a JSON array.");
  }
