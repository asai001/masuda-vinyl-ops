#!/usr/bin/env node
/**
 * Seed script for Payment Definitions Master (支払マスタ) TEST DATA (dev only)
 * Fixed settings:
 * - region: ap-northeast-1
 * - table : masuda-vinyl-ops-payment-definitions-master-dev
 * - data  : ./test_payment_definitions_master.json (relative to this script)
 *
 * Options:
...
const TABLE_NAME = "masuda-vinyl-ops-payment-definitions-master-dev";
const DATA_URL = new URL("./test_payment_definitions_master.json", import.meta.url);
...
async function main() {
  ...
  if (!Array.isArray(items)) {
    throw new Error("test_payment_definitions_master.json must be a JSON array.");
  }
