#!/usr/bin/env node
/**
 * Seed script for Products Master (bulk 100) (dev only)
 * Fixed:
 * - region: ap-northeast-1
 * - table : masuda-vinyl-ops-products-master-dev
 * - data  : ./seed_products_master_bulk.json (relative to this script)
 *
 * Options:
 *   --dry-run
...
const DATA_URL = new URL("./seed_products_master_bulk.json", import.meta.url);
...
function parseArgs(argv) {
  ...
  for (let i = 2; i < argv.length; i++) {
    ...
    if (a === "--help" || a === "-h") {
      console.log(`\nSeed Products Master (bulk 100) (dev)\n\nOptions:\n  --dry-run\n  --profile <profile>\n`);
      process.exit(0);
    }
...
async function main() {
  ...
  if (!Array.isArray(items)) {
    throw new Error("seed_products_master_bulk.json must be a JSON array.");
  }
