#!/usr/bin/env node
/**
 * Seed script for Settings (各種設定) TEST DATA (dev only)
 * Fixed settings:
 * - region: ap-northeast-1
 * - table : masuda-vinyl-ops-settings-dev
 * - data  : ./test_settings.json (relative to this script)
 *
 * Options:
...
const DATA_URL = new URL("./test_settings.json", import.meta.url);
...
async function main() {
  ...
  if (!Array.isArray(items)) {
    throw new Error("test_settings.json must be a JSON array.");
  }
