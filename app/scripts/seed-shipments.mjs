#!/usr/bin/env node
/**
 * Seed script for Shipments (dev only)
 * Fixed settings:
 * - region: ap-northeast-1
 * - table : masuda-vinyl-ops-shipments-dev
 * - data  : ./seed_shipments.json (relative to this script)
 *
 * Options:
...
const TABLE_NAME = "masuda-vinyl-ops-shipments-dev";
const SEQUENCES_TABLE_NAME = "masuda-vinyl-ops-sequences-dev";
const DATA_URL = new URL("./seed_shipments.json", import.meta.url);
...
function collectSequenceTargets(items) {
  const targets = new Map();

  for (const item of items) {
    const orgId = typeof item?.orgId === "string" ? item.orgId : "";
    const displayNo = typeof item?.displayNo === "number" && Number.isFinite(item.displayNo) ? item.displayNo : 0;
    if (!orgId || displayNo <= 0) {
      continue;
    }

    const current = targets.get(orgId) ?? 0;
    if (displayNo > current) {
      targets.set(orgId, displayNo);
    }
  }
...
async function ensureShipmentSequence(ddbDoc, items) {
  const targets = collectSequenceTargets(items);

  for (const [orgId, nextCurrent] of targets.entries()) {
    const res = await ddbDoc.send(
      new GetCommand({
        TableName: SEQUENCES_TABLE_NAME,
        Key: { orgId, sequenceName: "SH" },
      }),
    );

    const current = typeof res.Item?.current === "number" ? res.Item.current : 0;
    ...
    await ddbDoc.send(
      new UpdateCommand({
        ...
        ExpressionAttributeValues: { ":current": nextCurrent },
      }),
    );

    console.log(`[${SEQUENCES_TABLE_NAME}] synced SH current=${nextCurrent} for orgId=${orgId}`);
  }
}
...
async function main() {
  ...
  if (!Array.isArray(items)) {
    throw new Error("seed_shipments.json must be a JSON array.");
  }
  ...
  await ensureShipmentSequence(ddbDoc, items);
