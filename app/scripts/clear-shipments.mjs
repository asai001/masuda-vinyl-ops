#!/usr/bin/env node
/**
 * Clear script for Shipments (dev only)
 * Fixed:
  ...
  while (true) {
    const res = await ddbDoc.send(
      new ScanCommand({
        ...
        ProjectionExpression: "orgId, shipmentId",
        ExclusiveStartKey,
      }),
    );

    for (const item of res.Items ?? []) {
      if (item?.orgId && item?.shipmentId) {
        keys.push({ orgId: item.orgId, shipmentId: item.shipmentId });
      }
    }
