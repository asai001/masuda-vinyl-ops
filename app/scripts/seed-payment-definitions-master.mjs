#!/usr/bin/env node
/**
 * Seed script for Payment Definitions Master (支払マスタ) SEED DATA (dev only)
 * Fixed settings:
 * - region: ap-northeast-1
 * - table : masuda-vinyl-ops-payment-definitions-master-dev
 * - data  : ./seed_payment_definitions_master.json (relative to this script)
 *
 * Options:
...
const DATA_URL = new URL("./seed_payment_definitions_master.json", import.meta.url);
...
async function main() {
  ...
  if (!Array.isArray(items)) {
    throw new Error("seed_payment_definitions_master.json must be a JSON array.");
  }
