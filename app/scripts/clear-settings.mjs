#!/usr/bin/env node
/**
 * Clear script for Settings (各種設定) (dev only)
 * Fixed:
  ...
  while (true) {
    const res = await ddbDoc.send(
      new ScanCommand({
        ...
        ProjectionExpression: "orgId, settingsKey",
        ExclusiveStartKey,
      }),
    );

    for (const item of res.Items ?? []) {
      if (item?.orgId && item?.settingsKey) {
        keys.push({ orgId: item.orgId, settingsKey: item.settingsKey });
      }
    }
