#!/usr/bin/env node
/**
 * Seed script for Products Master (製品マスタ) SEED DATA (dev only)
 * Fixed settings:
 * - region: ap-northeast-1
 * - table : masuda-vinyl-ops-products-master-dev
 * - data  : ./seed_products_master.json (relative to this script)
 *
 * Options:
...
const DATA_URL = new URL("./seed_products_master.json", import.meta.url);
...
async function main() {
  ...
  if (!Array.isArray(items)) {
    throw new Error("seed_products_master.json must be a JSON array.");
  }
