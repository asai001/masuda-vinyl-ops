#!/usr/bin/env node
/**
 * Seed script for Purchase Orders (発注管理) TEST DATA (dev only)
 * Fixed settings:
 * - region: ap-northeast-1
 * - table : masuda-vinyl-ops-purchase-orders-dev
 * - data  : ./test_purchase_orders.json (relative to this script)
 *
 * Options:
...
const DATA_URL = new URL("./test_purchase_orders.json", import.meta.url);
...
async function main() {
  ...
  if (!Array.isArray(items)) {
    throw new Error("test_purchase_orders.json must be a JSON array.");
  }
