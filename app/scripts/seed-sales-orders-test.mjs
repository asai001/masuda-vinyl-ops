#!/usr/bin/env node
/**
 * Seed script for Sales Orders (受注管理) TEST DATA (dev only)
 * Fixed settings:
 * - region: ap-northeast-1
 * - table : masuda-vinyl-ops-sales-orders-dev
 * - data  : ./test_sales_orders.json (relative to this script)
 *
 * Options:
...
const DATA_URL = new URL("./test_sales_orders.json", import.meta.url);
...
async function main() {
  ...
  if (!Array.isArray(items)) {
    throw new Error("test_sales_orders.json must be a JSON array.");
  }
