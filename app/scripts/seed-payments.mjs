#!/usr/bin/env node
/**
 * Seed script for Payments (支払い管理) SEED DATA (dev only)
 * Fixed settings:
 * - region: ap-northeast-1
 * - table : masuda-vinyl-ops-payments-dev
 * - data  : ./seed_payments.json (relative to this script)
 *
 * Options:
...
const DATA_URL = new URL("./seed_payments.json", import.meta.url);
...
async function main() {
  ...
  if (!Array.isArray(items)) {
    throw new Error("seed_payments.json must be a JSON array.");
  }
