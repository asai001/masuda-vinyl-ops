...
export default function EditProductModal({
  ...
  const getInitialForm = (row: ProductRow | null) => ({
    code: row?.code ?? "",
    name: row?.name ?? "",
    packaging: row?.packaging === null || row?.packaging === undefined ? "" : String(row.packaging),
    ...
    length: row?.length === null || row?.length === undefined ? "" : String(row.length),
    speed: row?.speed === null || row?.speed === undefined ? "" : String(row.speed),
